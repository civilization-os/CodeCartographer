...
| src/index.ts | typescript | 0 | 4 | ./analyzer/analyzeRepo.js, ./docs/docsGenerator.js, ./llm/modelConfig.js, ./output/resultJsonWriter.js, node:path | src/index.ts 包含 0 个类、4 个方法单元和 5 个导入。 关键方法：解析命令行参数，执行代码仓库分析并生成文档，将结果写入JSON文件并输出日志。 解析命令行参数，提取命令、目标路径和模型配置。 验证命令行参数值是否存在且不以'--'开头，否则抛出错误。 |
...
| src/output/resultJsonWriter.ts | typescript | 0 | 9 | ../core/types.js, ../docs/qualityReport.js, ../docs/semanticAggregator.js, ../utils/path.js, node:fs/promises, node:path | src/output/resultJsonWriter.ts 包含 0 个类、9 个方法单元和 6 个导入。 关键方法：将结果数据写入指定目录下的 result.json 文件并返回文件路径。 将扫描结果、概览、质量数据和文档路径组装成标准化的 JSON 对象并返回。 将模块单元序列化为包含标识符、路径、语言、导入、摘要以及类和方法的ID列表的普通对象。 |
...
src/index.ts 包含 0 个类、4 个方法单元和 5 个导入。 关键方法：解析命令行参数，执行代码仓库分析并生成文档，将结果写入JSON文件并输出日志。 解析命令行参数，提取命令、目标路径和模型配置。 验证命令行参数值是否存在且不以'--'开头，否则抛出错误。
...
| Method | Signature | Return | Hints | Analyzer | Summary | Calls | Location |
| --- | --- | --- | --- | --- | --- | --- | --- |
| main | `main(): Promise<void>` | Promise<void> | cli | llm | 解析命令行参数，执行代码仓库分析并生成文档，将结果写入JSON文件并输出日志。 | analyzeRepo, console.log, filter, generateDocs, parseArgs, path.resolve, printHelp, process.argv.slice | src/index.ts:7 |
| parseArgs | `parseArgs(args: string[]): {   command: string;   targetPath: string;   modelConfig: ModelConfig; }` | {   command: string;   targetPath: string;   modelConfig: ModelConfig; } |  | llm | 解析命令行参数，提取命令、目标路径和模型配置。 | loadModelConfig, positional.push, requireValue | src/index.ts:45 |
| requireValue | `requireValue(flag: string, value: string \| undefined): string` | string |  | llm | 验证命令行参数值是否存在且不以'--'开头，否则抛出错误。 | value.startsWith | src/index.ts:101 |
| printHelp | `printHelp(): void` | void |  | llm | 打印 see-code 工具的使用说明和命令行参数帮助信息。 | console.log | src/index.ts:108 |
...
| Method | Signature | Return | Hints | Analyzer | Summary | Calls | Location |
| --- | --- | --- | --- | --- | --- | --- | --- |
| writeResultJson | `writeResultJson(input: ResultJsonInput): Promise<string>` | Promise<string> |  | llm | 将结果数据写入指定目录下的 result.json 文件并返回文件路径。 | JSON.stringify, fs.mkdir, fs.writeFile, path.join, toResultJson | src/output/resultJsonWriter.ts:28 |
| toResultJson | `toResultJson(input: ResultJsonInput): Record<string, unknown>` | Record<string, unknown> |  | llm | 将扫描结果、概览、质量数据和文档路径组装成标准化的 JSON 对象并返回。 | docs.map, group.modules.map, includes, item.methods.map, overview.businessFlows.map, overview.entrypoints.map, overview.flows.map, overview.hotMethods.map | src/output/resultJsonWriter.ts:37 |
| serializeModule | `serializeModule(module: ModuleUnit): Record<string, unknown>` | Record<string, unknown> |  | llm | 将模块单元序列化为包含标识符、路径、语言、导入、摘要以及类和方法的ID列表的普通对象。 | module.classes.map, module.methods.map | src/output/resultJsonWriter.ts:89 |
| serializeClass | `serializeClass(classUnit: ClassUnit): Record<string, unknown>` | Record<string, unknown> |  | llm | 将 ClassUnit 对象序列化为包含 id、name、modulePath、location、summary 和 methodIds 的普通对象。 | classUnit.methods.map | src/output/resultJsonWriter.ts:101 |
| serializeMethod | `serializeMethod(method: MethodUnit): Record<string, unknown>` | Record<string, unknown> |  | llm | 将 MethodUnit 对象序列化为一个包含其所有属性的普通 JSON 对象。 |  | src/output/resultJsonWriter.ts:112 |
| serializeEntrypoint | `serializeEntrypoint(entrypoint: EntrypointSummary): Record<string, unknown>` | Record<string, unknown> |  | llm | 将入口点摘要序列化为包含方法引用、扇出和原因的键值对对象。 | methodRef | src/output/resultJsonWriter.ts:137 |
| serializeFlow | `serializeFlow(flow: FlowSummary): Record<string, unknown>` | Record<string, unknown> |  | llm | 将FlowSummary对象序列化为包含名称、入口点、步骤和资源的普通对象。 | flow.steps.map, methodRef | src/output/resultJsonWriter.ts:145 |
| serializeBusinessFlow | `serializeBusinessFlow(flow: BusinessFlowCandidate): Record<string, unknown>` | Record<string, unknown> |  | llm | 将业务流候选对象序列化为包含名称、入口点、入口点提示、步骤和资源的普通对象。 | flow.steps.map, methodRef | src/output/resultJsonWriter.ts:154 |
| methodRef | `methodRef(method: MethodUnit): Record<string, unknown>` | Record<string, unknown> |  | llm | 将方法单元转换为包含标识符、名称、模块路径和位置的记录对象。 | formatMethodName | src/output/resultJsonWriter.ts:164 |

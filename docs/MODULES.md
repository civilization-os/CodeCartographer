...
| src/parser/javaStructureParser.ts | typescript | 0 | 52 | ../core/types.js, ../utils/path.js, node:fs/promises | src/parser/javaStructureParser.ts 包含 0 个类、52 个方法单元和 3 个导入。 关键方法：解析Java源文件并提取模块单元信息，包括类、方法和导入。 从Java源代码文本中提取所有导入语句并返回排序后的唯一导入列表。 从Java源代码中提取类、接口、枚举和记录的定义块，包括注解、声明和代码范围。 |
...
| hasRequiredDocs | `hasRequiredDocs(docs: Map<string, string>): boolean` | boolean |  | llm | 检查文档映射中是否包含所有必需的工程文档文件。 | docs.has, every | src/docs/qualityReport.ts:210 |
...
| printHelp | `printHelp(): void` | void |  | llm | 打印命令行帮助信息，展示使用方法和配置选项。 | console.log | src/index.ts:118 |
...
src/parser/javaStructureParser.ts 包含 0 个类、52 个方法单元和 3 个导入。 关键方法：解析Java源文件并提取模块单元信息，包括类、方法和导入。 从Java源代码文本中提取所有导入语句并返回排序后的唯一导入列表。 从Java源代码中提取类、接口、枚举和记录的定义块，包括注解、声明和代码范围。
...
显示 8/52 个高信号方法；完整方法级结构、调用和语义见 `.see-code/result.json`。
...
| extractClassResources | `extractClassResources(classBlock: JavaClassBlock): string[]` | string[] |  | llm | 从Java类块中提取实体、表和仓库资源标识符。 | annotationAttribute, annotationByName, exec, hasAnnotation, resources.add, sort, test | src/parser/javaStructureParser.ts:183 |
| buildClassResourceIndex | `buildClassResourceIndex(classBlocks: JavaClassBlock[]): Map<string, string[]>` | Map<string, string[]> |  | llm | 构建类名到资源列表的映射索引。 | classBlocks.map, extractClassResources | src/parser/javaStructureParser.ts:217 |
| buildMethodReturnTypeIndex | `buildMethodReturnTypeIndex(className: string, methodBlocks: JavaMethodBlock[]): MethodReturnTypeIndex` | MethodReturnTypeIndex |  | llm | 构建方法返回类型索引，将每个方法名称及其带类名前缀的变体映射到唯一的返回类型。 | candidates.entries, candidates.get, candidates.set, filter, map, normalizeJavaType | src/parser/javaStructureParser.ts:221 |
| repositoryOperationIntent | `repositoryOperationIntent(operation: string): string \| undefined` | string \| undefined |  | llm | 根据操作名称前缀返回对应的数据库操作意图（读、写或删除）。 | test | src/parser/javaStructureParser.ts:557 |

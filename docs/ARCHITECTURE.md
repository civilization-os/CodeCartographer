...
系统采用分层架构：CLI 层负责参数解析和流程编排，扫描层负责文件遍历和语言识别，解析层通过适配器模式将源文件转换为统一模块单元，图构建层提取调用关系，LLM 层增强语义，输出层序列化结果。各层通过模块单元（ModuleUnit）和关系图（RelationGraph）传递数据。

## Architecture Layers

| Layer | Modules | Responsibility |
| --- | --- | --- |
| CLI 层 | src/index.ts | 解析命令行参数，调用 analyzeRepo 执行全流程，触发 writeResultJson 输出结果。 |
| 扫描层 | src/scanner/repoScanner.ts | 递归扫描目录，过滤忽略文件、非文件、过大文件及无法识别语言的文件，返回排序后的源文件信息列表。 |
| 解析层 | src/parser/moduleParser.ts, src/parser/typescriptAdapter.ts, src/parser/typescriptStructureParser.ts, src/parser/javaAdapter.ts, src/parser/javaStructureParser.ts, src/parser/parserAdapter.ts | 遍历源文件，通过语言适配器解析为 ModuleUnit，包含导入、类、方法元数据。 |
| 图构建层 | src/graph/relationGraphBuilder.ts | 从模块单元中提取调用关系和资源引用，构建节点和边的关系图。 |
| LLM 语义层 | src/llm/methodSemanticAnalyzer.ts, src/llm/methodSemanticCache.ts, src/llm/modelConfig.ts, src/llm/modelFactory.ts | 对方法执行语义分析，优先使用缓存，未缓存则调用 LLM 或启发式规则，更新模块和类摘要。 |
| 输出层 | src/output/resultJsonWriter.ts | 将扫描结果、关系图、语义标签组装为标准化 JSON，写入文件系统（result.json、result-diff.json、CHANGE_SUMMARY.md）。 |
| 工具层 | src/utils/path.ts | 提供路径规范化（POSIX 风格）和稳定标识符生成函数。 |
...
| Application | 1 | 4 | 解析命令行参数，执行代码仓库分析并生成文档，输出结果到文件。 |
...
| objectRecord | src/output/resultJsonWriter.ts | 将未知类型值转换为记录对象，若非对象或为数组则返回空对象。 |
| routeFromAnnotations | src/parser/javaStructureParser.ts | 从注解列表中解析出HTTP方法和路径，结合类路由前缀生成完整路由。 |
| writeResultJson | src/output/resultJsonWriter.ts | 将结果写入文件系统，包括结果JSON、差异JSON和变更摘要Markdown文件。 |
| files | src/output/resultJsonWriter.ts | 从结果对象中提取文件列表并转换为文件差异条目数组。 |
| methods | src/output/resultJsonWriter.ts | 从结果记录中提取方法列表，映射为包含指纹信息的方法差异条目，并过滤掉缺少名称或模块路径的条目。 |

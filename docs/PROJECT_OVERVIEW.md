...
see-code 是一个静态代码仓库分析工具，用于扫描指定仓库的源代码，提取模块、方法、类、资源及调用关系图，并自动生成结构化的工程文档（Markdown 格式）和 JSON 结果文件。

## Operating Model

1. 用户通过 CLI 传入目标仓库路径和可选参数。
2. 工具递归扫描仓库文件，过滤排除项和大文件，识别源代码语言。
3. 解析器模块将源文件解析为模块、类、方法的结构化单元。
4. 关系图构建器根据调用字符串和方法索引建立节点与边的关系图。
5. 语义分析器通过启发式规则或 LLM 为每个方法附加语义标签。
6. 文档生成器将分析结果渲染为多个 Markdown 文件并写入输出目录。
7. 结果写入器将扫描结果、差异报告和变更摘要输出到文件系统。

## Key Capabilities

- 支持 TypeScript/JavaScript 和 Java 源代码解析。
- 构建模块、类、方法和资源之间的静态调用关系图。
- 通过 LLM 或启发式方法为方法附加语义标签。
- 生成项目概览、架构文档、模块文档、质量报告等工程文档。
- 支持结果差异比较和变更摘要输出。
- 可配置的排除规则、最大文件大小和模型参数。

...
Scan time: `2026-06-04T05:47:09.437Z`

## Purpose

see-code 是一个静态代码仓库分析工具，扫描指定仓库的源文件，提取模块、方法、类、资源及调用关系图，并生成结构化工程文档（Markdown）和 JSON 结果。
...
4. 对每个方法执行语义分析（优先缓存，未缓存则调用 LLM 或启发式规则），更新模块和类摘要。
5. 生成项目概览、架构文档、模块列表、质量报告等 Markdown 文件，并输出结果 JSON 和差异 JSON。

## Key Capabilities

- 支持 TypeScript/JavaScript 和 Java 源文件解析。
- 构建静态调用关系图，包含模块、类、方法和资源节点。
- 通过 LLM 或启发式规则为每个方法附加语义标签。
- 生成多维度工程文档（概览、架构、模块、质量报告）。

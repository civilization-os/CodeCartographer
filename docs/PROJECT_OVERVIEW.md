...
Scan time: `2026-06-04T03:20:37.035Z`

## Purpose

see-code 是一个静态代码仓库分析工具，用于扫描指定仓库的源代码，提取模块、类、方法、资源及调用关系图，并自动生成工程文档（Markdown 格式）和结构化 JSON 结果。

## Operating Model

1. 用户通过 CLI 调用 see-code，传入目标仓库路径和可选参数。
2. 工具解析命令行参数，加载项目配置和 LLM 模型配置。
3. 递归扫描目标目录，过滤排除项、大文件和未知语言文件，获取源文件列表。
4. 解析每个源文件，提取模块、类、方法及其元数据（导入、位置、签名等）。
5. 构建模块、类、方法和资源之间的关系图（节点和边）。
6. 对每个方法进行语义分析（优先使用缓存，未缓存则调用 LLM 或启发式方法）。
7. 生成工程文档（项目概览、架构、模块、方法语义等）并写入 Markdown 文件。
8. 将分析结果、差异报告和变更摘要写入 JSON 文件，同时输出到控制台。

## Key Capabilities

- 支持 TypeScript/JavaScript 和 Java 源代码解析。
- 自动构建静态调用关系图。
- 集成 LLM 进行方法级语义分析，支持缓存和降级策略。
- 生成结构化工程文档（Markdown）和 JSON 结果。
- 支持结果差异比较，用于增量分析。
- 可配置排除规则、文件大小限制和语言检测。
...
| Project Files | schema/result-diff.schema.json, schema/result.schema.json, scripts/secret-scan.mjs, tests/analyzeRepo.test.ts, tests/schemaContract.test.ts | 递归扫描目录，读取文件内容并检测是否匹配预定义的密钥模式，将匹配结果记录到数组中。 从相对于仓库根目录的路径异步读取并解析JSON文件。 递归验证未知值是否符合给定的 JSON Schema 约束，包括常量、类型、必需属性、禁止属性及嵌套属性。 |
...
| Exclude rules | .git/**, .see-code/**, node_modules/**, dist/**, docs/**, build/**, coverage/**, .next/**, .turbo/**, .cache/**, target/**, __pycache__/**, tests/fixtures/** |

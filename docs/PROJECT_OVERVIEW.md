...
Scan time: `2026-06-04T05:52:24.074Z`

## Purpose

see-code 是一个静态代码仓库分析工具，扫描指定仓库的源代码文件，提取模块、类、方法、资源及调用关系图，生成结构化工程文档和 Markdown 报告，并支持通过 LLM 为方法附加语义标签。

## Operating Model

1. CLI 入口解析命令行参数，加载项目配置和 LLM 模型配置。
2. 递归扫描仓库目录，过滤排除项和大文件，按语言类型解析源文件为模块单元。
3. 构建模块、类、方法和资源之间的关系图，生成调用关系边。
4. 对每个方法执行语义分析（优先缓存，未命中则调用 LLM 或启发式规则）。
5. 将分析结果组装为结构化 JSON，写入文件系统，并生成多份 Markdown 文档。

## Key Capabilities

- 支持 TypeScript/JavaScript 和 Java 源文件的结构化解析。
- 构建静态调用关系图，包含节点和边。
- 通过 LLM 或启发式规则为方法附加语义标签。
- 生成项目概览、架构文档、模块文档、质量报告等 Markdown 文件。
- 输出结果 JSON 和差异 JSON，支持增量变更追踪。

...
Scan time: `2026-06-04T07:14:40.538Z`

## Purpose

see-code 是一个静态代码仓库分析工具，扫描指定目录下的源文件，提取模块、方法、类、资源及调用关系图，生成结构化工程文档（Markdown 和 JSON），并可选通过 LLM 增强方法语义标签。

## Operating Model

1. CLI 入口解析命令行参数，加载项目配置和 LLM 模型配置。
2. 递归扫描目标目录，过滤排除项、大文件和未知语言文件，返回源文件列表。
3. 按语言适配器解析每个源文件，提取模块单元（导入、类、方法、资源）。
4. 构建模块、类、方法和资源之间的关系图（节点和边）。
5. 对每个方法执行语义分析（缓存优先，未命中则调用 LLM 或启发式规则）。
6. 根据分析结果生成多份 Markdown 文档（概览、架构、模块、业务流等）并写入文件系统。
7. 将完整结果（含差异和变更摘要）序列化为 JSON 并输出到控制台和文件。

## Key Capabilities

- 支持 TypeScript 和 Java 源文件的结构解析。
- 生成静态调用关系图，包含模块、类、方法和资源节点。
- 通过 LLM 或启发式规则为方法附加语义标签（如数据访问、业务逻辑、路由处理）。
- 自动生成工程文档（架构、模块、业务流、质量报告）。
- 支持结果差异比较，输出变更摘要。
- 可配置排除规则、最大文件大小和 LLM 模型参数。

...
Scan time: `2026-06-04T06:09:43.287Z`

## Purpose

see-code 是一个静态代码分析工具，用于扫描指定代码仓库，提取模块、方法、类、资源及调用关系图，并自动生成结构化工程文档（Markdown 格式）和 JSON 分析结果。

## Operating Model

1. CLI 入口解析命令行参数，加载项目配置和 LLM 模型配置。
2. 调用 analyzeRepo 递归扫描源文件，解析模块结构，构建关系图。
3. 对每个方法执行语义分析（优先缓存，未命中则调用 LLM 或启发式规则）。
4. 生成 Markdown 文档（项目概览、架构、模块、业务流等）并写入文件系统。
5. 输出结果 JSON、差异 JSON 和变更摘要到指定目录。

## Key Capabilities

- 递归扫描目录，按排除规则过滤文件，检测源代码语言类型。
- 解析 TypeScript/JavaScript 和 Java 源文件，提取导入、类、方法及资源标识。
- 构建包含模块、类、方法和资源层级的关系图，支持调用关系分析。
- 通过 LLM 或启发式方法为方法附加语义标签，生成方法摘要。
- 生成完整的工程文档（项目概览、架构层、模块表格、质量报告等）。
- 支持结果差异比较，输出结构化变更摘要。
...
| graph | src/graph/relationGraphBuilder.ts | 根据模块单元和资源节点构建包含模块、类、方法和资源层级的关系图，并返回去重后的节点和边集合。 从模块单元中提取所有资源并去重排序，返回资源节点数组。 构建方法名称到方法单元的索引映射，支持类名限定和多种命名格式。 |

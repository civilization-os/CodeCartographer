...
Scan time: `2026-06-04T06:54:04.914Z`

## Purpose

see-code 是一个静态代码仓库分析工具，扫描指定目录下的源代码文件，提取模块、方法、类、资源及调用关系图，生成结构化工程文档（Markdown 和 JSON），并可选通过 LLM 补充方法语义标签。

## Operating Model

1. CLI 入口解析命令行参数，加载项目配置和 LLM 模型配置。
2. 递归扫描仓库目录，过滤排除项和大文件，按扩展名识别语言类型。
3. 解析源文件提取模块单元（导入、类、方法、资源），构建关系图。
4. 对每个方法执行语义分析（缓存优先，未命中则调用 LLM 或启发式规则）。
5. 生成多份 Markdown 文档（概览、架构、模块、业务流、质量报告）并写入文件系统。
6. 将分析结果序列化为 JSON 并输出到控制台和文件。

## Key Capabilities

- 支持 Java 和 TypeScript 源代码的结构解析。
- 构建模块-类-方法-资源四层关系图。
- 通过 LLM 或启发式规则为方法附加语义标签。
- 生成包含架构层、关键路径、热点方法的工程文档。
- 支持结果差异比较和变更摘要输出。
- 可配置的排除规则、最大文件大小和 LLM 模型参数。

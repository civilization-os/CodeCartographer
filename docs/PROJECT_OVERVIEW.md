...
Scan time: `2026-06-04T06:30:08.568Z`

## Purpose

see-code 是一个静态代码仓库分析工具，扫描指定目录下的源代码文件，提取模块、类、方法、资源及调用关系图，生成结构化工程文档和 Markdown 报告。

## Operating Model

1. CLI 入口解析命令行参数，加载项目配置和 LLM 模型配置。
2. 调用 repoScanner 递归扫描目录，过滤排除项和大文件，返回源文件列表。
3. 调用 analyzeRepo 解析每个源文件，提取模块、类、方法及资源，构建关系图。
4. 调用 methodSemanticAnalyzer 对每个方法进行语义分析（优先缓存，未命中则通过 LLM 或启发式规则）。
5. 调用 docsGenerator 将分析结果渲染为多个 Markdown 文档并写入输出目录。
6. 调用 resultJsonWriter 将结果序列化为 JSON 文件，并生成差异报告和变更摘要。

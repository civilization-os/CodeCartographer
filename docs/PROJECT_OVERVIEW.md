...
Scan time: `2026-06-04T02:32:22.832Z`

## Purpose

see-code 是一个静态代码分析工具，用于扫描指定代码仓库，提取模块、方法、类、资源及关系图，并生成工程文档。

## Operating Model

1. 用户通过 CLI 调用 see-code，传入目标仓库路径和可选参数。
2. 工具解析命令行参数，加载项目配置和 LLM 模型配置。
3. 递归扫描目标目录，过滤排除项、大文件和未知语言文件，返回源文件列表。
4. 解析每个源文件，提取模块、类、方法及其导入关系。
5. 构建模块、类、方法和资源之间的关系图。
6. 对每个方法进行语义分析（优先使用缓存，未缓存则调用 LLM 或启发式方法）。
7. 生成工程文档（Markdown 文件）并写入指定目录。
8. 将结果序列化为 JSON 并写入文件系统，同时输出差异报告和变更摘要。

## Key Capabilities

- 支持 TypeScript 和 JavaScript 源代码解析，Java 适配器在规划中。
- 自动构建静态调用关系图，包含节点和边。
- 通过 LLM 或启发式规则为方法附加语义标签。
- 生成项目概览、架构文档、模块列表、质量报告等 Markdown 文档。
- 支持结果差异比较，输出结构化差异报告。
...
| Project Files | scripts/secret-scan.mjs, tests/analyzeRepo.test.ts | 递归扫描目录，读取文件内容并检测是否匹配预定义的密钥模式，将匹配结果记录到数组中。 |

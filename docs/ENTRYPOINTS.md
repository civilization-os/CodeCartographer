...
| --- | --- | --- | --- | --- | --- |
| main | src/index.ts | cli | cli | main function | 解析命令行参数，执行代码仓库分析并生成文档，将结果写入JSON文件并输出日志。 |
...
| main | src/index.ts | 4 | 没有已解析的内部调用者，并且存在已解析的下游调用。 | 解析命令行参数，执行代码仓库分析并生成文档，将结果写入JSON文件并输出日志。 |

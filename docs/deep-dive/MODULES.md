...
| src/cli/analyzeCommand.ts | typescript | 0 | 1 | ../analyzer/analyzeRepo.js, ../config/projectConfig.js, ../docs/docsGenerator.js, ../llm/modelConfig.js, ../output/resultJsonWriter.js, node:path | src/cli/analyzeCommand.ts 包含 0 个类、1 个方法单元和 6 个导入。 关键方法：runAnalyzeCommand 定义一个可调用单元；调用 analyzeRepo, applyModelNetworkEnv, console.log, generateDocs, loadModelConfig；访问 FILE:QUALITY_REPORT.md, FILE:README.md。 |
| src/cli/args.ts | typescript | 0 | 5 | ../core/types.js | src/cli/args.ts 包含 0 个类、5 个方法单元和 1 个导入。 关键方法：parseCliArgs 定义一个可调用单元；调用 COMMANDS.has, commandFrom, excludes.push, parsePositiveInteger, positional.push。 将字符串或未定义值标准化为有效的ModelProvider枚举值，若无效则抛出错误。 commandFrom 定义一个可调用单元；调用 COMMANDS.has。 |
| src/cli/doctorCommand.ts | typescript | 0 | 4 | ../config/projectConfig.js, ../llm/modelConfig.js, node:fs/promises, node:path | src/cli/doctorCommand.ts 包含 0 个类、4 个方法单元和 4 个导入。 关键方法：运行医生诊断命令，收集并输出所有检查结果，若有失败项则设置退出码为1。 collectDoctorChecks 定义一个可调用单元；调用 String, checks.push, isDirectory, loadModelConfig, loadProjectConfig。 异步检查给定路径是否为目录，如果路径不存在或发生错误则返回 false。 |
| src/cli/initCommand.ts | typescript | 0 | 4 | ../config/projectConfig.js, ../core/types.js, ../scanner/repoScanner.js, node:fs/promises, node:path | src/cli/initCommand.ts 包含 0 个类、4 个方法单元和 5 个导入。 关键方法：初始化项目配置并写入文件，同时输出提示信息。 在指定路径下创建项目配置文件，若文件已存在且未使用强制覆盖选项则抛出异常。 buildProjectConfig 定义一个可调用单元。 |
| src/cli/interactiveCommand.ts | typescript | 0 | 5 | ../config/projectConfig.js, ../llm/modelConfig.js, ./analyzeCommand.js, ./args.js, ./doctorCommand.js, ./initCommand.js | src/cli/interactiveCommand.ts 包含 0 个类、5 个方法单元和 8 个导入。 关键方法：runInteractiveCommand 定义一个可调用单元；调用 ask, askSecret, askYesNo, console.log, defaultModel。 异步读取用户输入的密码或敏感信息，在终端中隐藏输入内容并支持退格和中断处理。 向用户提出一个问题并返回答案，如果用户未输入则返回默认值。 |
| src/cli/modelCommand.ts | typescript | 0 | 7 | ../config/projectConfig.js, ../llm/modelConfig.js, ../llm/modelFactory.js, node:fs/promises, node:path, node:process | src/cli/modelCommand.ts 包含 0 个类、7 个方法单元和 7 个导入。 关键方法：runModelTestCommand 定义一个可调用单元；调用 Date.now, applyModelNetworkEnv, askYesNo, console.log, createChatModel。 shouldSave 定义一个可调用单元。 saveModelDefaults 定义一个可调用单元；调用 JSON.stringify, fs.mkdir, fs.writeFile, path.join。 |
...
| src/docs/qualityReport.ts | typescript | 0 | 10 | ../core/types.js, ./markdown.js, ./narrativeComposer.js, ./semanticAggregator.js | src/docs/qualityReport.ts 包含 0 个类、10 个方法单元和 4 个导入。 关键方法：根据分析结果、语义概览、项目叙事和文档生成质量报告的字符串表示。 根据分析结果、语义概览、项目叙事和文档生成质量摘要，包含检查项、评分和推荐建议。 buildChecks 定义一个可调用单元；调用 countMatches, docs.values, hasRequiredDocs, isChineseNarrative, join。 |
...
| src/index.ts | typescript | 0 | 2 | ./cli/analyzeCommand.js, ./cli/args.js, ./cli/doctorCommand.js, ./cli/initCommand.js, ./cli/interactiveCommand.js, ./cli/modelCommand.js | src/index.ts 包含 0 个类、2 个方法单元和 6 个导入。 关键方法：main 定义一个可调用单元；调用 normalizeProvider, parseCliArgs, printHelp, process.argv.slice, runAnalyzeCommand。 printHelp 定义一个可调用单元；调用 console.log。 |
...
| src/llm/modelConfig.ts | typescript | 0 | 9 | ../config/projectConfig.js, ../core/types.js, zod | src/llm/modelConfig.ts 包含 0 个类、9 个方法单元和 3 个导入。 关键方法：loadModelConfig 定义一个可调用单元；调用 Boolean, defaultBaseUrl, defaultModel, getApiKey, modelProviderSchema.parse。 applyModelNetworkEnv 定义一个可调用单元；访问 ENV:NO_PROXY, ENV:no_proxy。 将ModelConfig对象转换为ModelRuntimeInfo对象，并根据provider字段决定是否保留model字段。 |
| src/llm/modelFactory.ts | typescript | 0 | 1 | ./modelConfig.js, @langchain/anthropic, @langchain/core/messages, @langchain/openai | src/llm/modelFactory.ts 包含 0 个类、1 个方法单元和 4 个导入。 关键方法：createChatModel 定义一个可调用单元；调用 applyModelNetworkEnv。 |
| src/output/resultJsonWriter.ts | typescript | 0 | 32 | ../core/types.js, ../docs/qualityReport.js, ../docs/semanticAggregator.js, ../utils/path.js, node:fs/promises, node:path | src/output/resultJsonWriter.ts 包含 0 个类、32 个方法单元和 6 个导入。 关键方法：writeResultJson 定义一个可调用单元；调用 JSON.stringify, buildResultDiff, fs.mkdir, fs.writeFile, path.dirname；访问 FILE:CHANGE_SUMMARY.md, FILE:result-diff.json, FILE:result.json。 将扫描结果、概览、质量数据和文档路径组装为结构化的 JSON 对象并返回。 将模块单元序列化为包含标识符、路径、语言、导入、摘要以及类和方法的ID列表的普通对象。 |
...
| Method | Signature | Return | Hints | Analyzer | Summary | Calls | Location |
| --- | --- | --- | --- | --- | --- | --- | --- |
| runAnalyzeCommand | `runAnalyzeCommand(options: AnalyzeCommandOptions): Promise<void>` | Promise<void> |  | heuristic | runAnalyzeCommand 定义一个可调用单元；调用 analyzeRepo, applyModelNetworkEnv, console.log, generateDocs, loadModelConfig；访问 FILE:QUALITY_REPORT.md, FILE:README.md。 | analyzeRepo, applyModelNetworkEnv, console.log, generateDocs, loadModelConfig, loadProjectConfig, path.join, path.resolve | src/cli/analyzeCommand.ts:13 |
## src/cli/args.ts

src/cli/args.ts 包含 0 个类、5 个方法单元和 1 个导入。 关键方法：parseCliArgs 定义一个可调用单元；调用 COMMANDS.has, commandFrom, excludes.push, parsePositiveInteger, positional.push。 将字符串或未定义值标准化为有效的ModelProvider枚举值，若无效则抛出错误。 commandFrom 定义一个可调用单元；调用 COMMANDS.has。
...
| Method | Signature | Return | Hints | Analyzer | Summary | Calls | Location |
| --- | --- | --- | --- | --- | --- | --- | --- |
| parseCliArgs | `parseCliArgs(rawArgs: string[]): CliOptions` | CliOptions |  | heuristic | parseCliArgs 定义一个可调用单元；调用 COMMANDS.has, commandFrom, excludes.push, parsePositiveInteger, positional.push。 | COMMANDS.has, commandFrom, excludes.push, parsePositiveInteger, positional.push, rawArgs.filter, requireValue | src/cli/args.ts:18 |
| normalizeProvider | `normalizeProvider(value: string \| undefined): ModelProvider` | ModelProvider |  | llm | 将字符串或未定义值标准化为有效的ModelProvider枚举值，若无效则抛出错误。 |  | src/cli/args.ts:146 |
| commandFrom | `commandFrom(value: string \| undefined): CliCommand` | CliCommand |  | heuristic | commandFrom 定义一个可调用单元；调用 COMMANDS.has。 | COMMANDS.has | src/cli/args.ts:163 |
| requireValue | `requireValue(flag: string, value: string \| undefined): string` | string |  | llm | 验证命令行标志参数值是否存在且不以'--'开头，否则抛出错误。 | value.startsWith | src/cli/args.ts:176 |
| parsePositiveInteger | `parsePositiveInteger(flag: string, value: string): number` | number |  | llm | 解析命令行参数中的正整数，若无效则抛出错误。 | Number.isFinite, Number.parseInt | src/cli/args.ts:183 |
...
| collectDoctorChecks | `collectDoctorChecks(options: DoctorCommandOptions): Promise<DoctorCheck[]>` | Promise<DoctorCheck[]> |  | heuristic | collectDoctorChecks 定义一个可调用单元；调用 String, checks.push, isDirectory, loadModelConfig, loadProjectConfig。 | String, checks.push, isDirectory, loadModelConfig, loadProjectConfig, path.resolve | src/cli/doctorCommand.ts:30 |
| isDirectory | `isDirectory(filePath: string): Promise<boolean>` | Promise<boolean> |  | llm | 异步检查给定路径是否为目录，如果路径不存在或发生错误则返回 false。 | fs.stat, isDirectory | src/cli/doctorCommand.ts:95 |
| formatStatus | `formatStatus(status: DoctorCheck["status"]): string` | string |  | llm | 将医生检查状态枚举值转换为对应的格式化字符串标签。 |  | src/cli/doctorCommand.ts:103 |
## src/cli/initCommand.ts

src/cli/initCommand.ts 包含 0 个类、4 个方法单元和 5 个导入。 关键方法：初始化项目配置并写入文件，同时输出提示信息。 在指定路径下创建项目配置文件，若文件已存在且未使用强制覆盖选项则抛出异常。 buildProjectConfig 定义一个可调用单元。
...
| Method | Signature | Return | Hints | Analyzer | Summary | Calls | Location |
| --- | --- | --- | --- | --- | --- | --- | --- |
| runInitCommand | `runInitCommand(options: InitCommandOptions): Promise<void>` | Promise<void> |  | llm | 初始化项目配置并写入文件，同时输出提示信息。 | console.log, writeProjectConfig | src/cli/initCommand.ts:18 |
| writeProjectConfig | `writeProjectConfig(options: InitCommandOptions): Promise<string>` | Promise<string> |  | llm | 在指定路径下创建项目配置文件，若文件已存在且未使用强制覆盖选项则抛出异常。 | JSON.stringify, buildProjectConfig, exists, fs.mkdir, fs.writeFile, path.join, path.resolve | src/cli/initCommand.ts:24 |
| buildProjectConfig | `buildProjectConfig(options: InitCommandOptions): ProjectConfig` | ProjectConfig |  | heuristic | buildProjectConfig 定义一个可调用单元。 |  | src/cli/initCommand.ts:38 |
| exists | `exists(filePath: string): Promise<boolean>` | Promise<boolean> |  | llm | 检查指定文件路径是否存在。 | fs.access | src/cli/initCommand.ts:65 |
## src/cli/interactiveCommand.ts

src/cli/interactiveCommand.ts 包含 0 个类、5 个方法单元和 8 个导入。 关键方法：runInteractiveCommand 定义一个可调用单元；调用 ask, askSecret, askYesNo, console.log, defaultModel。 异步读取用户输入的密码或敏感信息，在终端中隐藏输入内容并支持退格和中断处理。 向用户提出一个问题并返回答案，如果用户未输入则返回默认值。
...
| Method | Signature | Return | Hints | Analyzer | Summary | Calls | Location |
| --- | --- | --- | --- | --- | --- | --- | --- |
| runInteractiveCommand | `runInteractiveCommand(): Promise<void>` | Promise<void> |  | heuristic | runInteractiveCommand 定义一个可调用单元；调用 ask, askSecret, askYesNo, console.log, defaultModel。 | ask, askSecret, askYesNo, console.log, defaultModel, loadModelConfig, loadProjectConfig, normalizeProvider | src/cli/interactiveCommand.ts:10 |
| askSecret | `askSecret(rl: readline.Interface, question: string): Promise<string>` | Promise<string> |  | llm | 异步读取用户输入的密码或敏感信息，在终端中隐藏输入内容并支持退格和中断处理。 | ask, chunk.toString, cleanup, finish, reject, resolve, stdin.off, stdin.on | src/cli/interactiveCommand.ts:83 |
| ask | `ask(rl: readline.Interface, question: string, defaultValue: string): Promise<string>` | Promise<string> |  | llm | 向用户提出一个问题并返回答案，如果用户未输入则返回默认值。 | rl.question, trim | src/cli/interactiveCommand.ts:136 |
| askYesNo | `askYesNo(rl: readline.Interface, question: string, defaultValue: boolean): Promise<boolean>` | Promise<boolean> |  | llm | 通过命令行交互询问用户是或否的问题，并返回布尔值结果。 | rl.question, toLowerCase, trim | src/cli/interactiveCommand.ts:146 |
| defaultModel | `defaultModel(provider: string): string` | string |  | llm | 根据提供商名称返回默认模型标识符。 |  | src/cli/interactiveCommand.ts:158 |
## src/cli/modelCommand.ts

src/cli/modelCommand.ts 包含 0 个类、7 个方法单元和 7 个导入。 关键方法：runModelTestCommand 定义一个可调用单元；调用 Date.now, applyModelNetworkEnv, askYesNo, console.log, createChatModel。 shouldSave 定义一个可调用单元。 saveModelDefaults 定义一个可调用单元；调用 JSON.stringify, fs.mkdir, fs.writeFile, path.join。
...
| Method | Signature | Return | Hints | Analyzer | Summary | Calls | Location |
| --- | --- | --- | --- | --- | --- | --- | --- |
| runModelTestCommand | `runModelTestCommand(options: ModelTestCommandOptions): Promise<void>` | Promise<void> |  | heuristic | runModelTestCommand 定义一个可调用单元；调用 Date.now, applyModelNetworkEnv, askYesNo, console.log, createChatModel。 | Date.now, applyModelNetworkEnv, askYesNo, console.log, createChatModel, loadModelConfig, loadProjectConfig, model.invoke | src/cli/modelCommand.ts:24 |
| shouldSave | `shouldSave(options: ModelTestCommandOptions): boolean` | boolean |  | heuristic | shouldSave 定义一个可调用单元。 |  | src/cli/modelCommand.ts:75 |
| saveModelDefaults | `saveModelDefaults(rootPath: string, existingConfig: ProjectConfig, modelConfig: ModelConfig): Promise<void>` | Promise<void> |  | heuristic | saveModelDefaults 定义一个可调用单元；调用 JSON.stringify, fs.mkdir, fs.writeFile, path.join。 | JSON.stringify, fs.mkdir, fs.writeFile, path.join | src/cli/modelCommand.ts:79 |
| previewResponse | `previewResponse(response: unknown): string` | string |  | heuristic | previewResponse 定义一个可调用单元；调用 JSON.stringify, isMessageLike, truncate。 | JSON.stringify, isMessageLike, truncate | src/cli/modelCommand.ts:112 |
| isMessageLike | `isMessageLike(value: unknown): value is { content: unknown }` | value is { content: unknown } |  | heuristic | isMessageLike 定义一个可调用单元。 |  | src/cli/modelCommand.ts:122 |
| truncate | `truncate(value: string \| undefined): string` | string |  | heuristic | truncate 定义一个可调用单元；调用 replace, text.slice, trim。 | replace, text.slice, trim | src/cli/modelCommand.ts:126 |
| askYesNo | `askYesNo(rl: readline.Interface, question: string, defaultValue: boolean): Promise<boolean>` | Promise<boolean> |  | heuristic | askYesNo 定义一个可调用单元；调用 rl.question, toLowerCase, trim。 | rl.question, toLowerCase, trim | src/cli/modelCommand.ts:131 |
...
| Method | Signature | Return | Hints | Analyzer | Summary | Calls | Location |
| --- | --- | --- | --- | --- | --- | --- | --- |
| loadProjectConfig | `loadProjectConfig(rootPath: string): Promise<LoadedProjectConfig>` | Promise<LoadedProjectConfig> |  | llm | 从指定根路径异步加载并解析项目配置文件，若文件不存在则返回空配置。 | JSON.parse, fs.readFile, isNodeError, path.join, projectConfigSchema.parse, rejectSensitiveKeys | src/config/projectConfig.ts:41 |
| rejectSensitiveKeys | `rejectSensitiveKeys(value: unknown): void` | void |  | llm | 递归遍历对象并检查是否包含敏感键名，若发现则抛出错误。 | Array.isArray, Object.values, rejectSensitiveKeys | src/config/projectConfig.ts:65 |
| isNodeError | `isNodeError(error: unknown): error is NodeJS.ErrnoException` | error is NodeJS.ErrnoException |  | llm | 判断未知错误是否为 Node.js 的 ErrnoException 类型。 |  | src/config/projectConfig.ts:79 |
## src/docs/docsGenerator.ts

src/docs/docsGenerator.ts 包含 0 个类、20 个方法单元和 8 个导入。 关键方法：generateDocs 定义一个可调用单元；调用 buildQualitySummary, buildSemanticOverview, composeProjectNarrative, content.trim, docs.set；访问 FILE:README.md, FILE:ai/AI_CONTEXT.md, FILE:deep-dive/CALL_GRAPH.md, FILE:deep-dive/DATA_AND_RESOURCES.md, FILE:deep-dive/ENTRYPOINTS.md。 renderDocIndex 定义一个可调用单元；调用 String, bulletList, docRows.map, docs.get, docs.has；访问 FILE:ai/AI_CONTEXT.md, FILE:deep-dive/CALL_GRAPH.md, FILE:deep-dive/CHANGE_SUMMARY.md, FILE:deep-dive/DATA_AND_RESOURCES.md, FILE:deep-dive/ENTRYPOINTS.md。 renderSystemMap 定义一个可调用单元；调用 String, bulletList, corePipeline.slice, formatMethodName, group.modules.some。
...
显示 8/20 个高信号方法；完整方法级结构、调用和语义见 `.see-code/result.json`。

| Method | Signature | Return | Hints | Analyzer | Summary | Calls | Location |
| --- | --- | --- | --- | --- | --- | --- | --- |
| generateDocs | `generateDocs(result: AnalysisResult): Promise<GeneratedDocs>` | Promise<GeneratedDocs> |  | heuristic | generateDocs 定义一个可调用单元；调用 buildQualitySummary, buildSemanticOverview, composeProjectNarrative, content.trim, docs.set；访问 FILE:README.md, FILE:ai/AI_CONTEXT.md, FILE:deep-dive/CALL_GRAPH.md, FILE:deep-dive/DATA_AND_RESOURCES.md, FILE:deep-dive/ENTRYPOINTS.md。 | buildQualitySummary, buildSemanticOverview, composeProjectNarrative, content.trim, docs.set, fs.mkdir, fs.writeFile, path.dirname | src/docs/docsGenerator.ts:26 |
| renderDocIndex | `renderDocIndex(result: AnalysisResult, overview: SemanticOverview, narrative: ProjectNarrative, quality: QualitySummary, docs: Map<string, string>): string` | string |  | heuristic | renderDocIndex 定义一个可调用单元；调用 String, bulletList, docRows.map, docs.get, docs.has；访问 FILE:ai/AI_CONTEXT.md, FILE:deep-dive/CALL_GRAPH.md, FILE:deep-dive/CHANGE_SUMMARY.md, FILE:deep-dive/DATA_AND_RESOURCES.md, FILE:deep-dive/ENTRYPOINTS.md。 | String, bulletList, docRows.map, docs.get, docs.has, heading, isInternalResource, join | src/docs/docsGenerator.ts:70 |
| renderArchitecture | `renderArchitecture(result: AnalysisResult, overview: SemanticOverview, narrative: ProjectNarrative): string` | string |  | llm | 生成架构文档的完整 Markdown 字符串，包含系统形状、架构层、关键路径、模块区域、核心热点方法和运行时资源等章节。 | String, bulletList, flatMap, formatMethodName, group.modules.reduce, heading, isInternalResource, join | src/docs/docsGenerator.ts:475 |
| renderModules | `renderModules(modules: ModuleUnit[]): string` | string |  | llm | 生成模块文档的 Markdown 字符串，包含模块概览表格和每个模块的详细信息（导入、类、高信号方法）。 | String, bulletList, flatMap, formatMethodHints, heading, join, lines.join, lines.push | src/docs/docsGenerator.ts:544 |
| selectHighSignalMethods | `selectHighSignalMethods(methods: ModuleUnit["methods"], limit: number): ModuleUnit["methods"]` | ModuleUnit["methods"] |  | llm | 根据评分和索引排序，选择前N个高信号方法并保持原始顺序。 | map, methods     .map, scoreMethodForModuleDocs, slice, sort | src/docs/docsGenerator.ts:628 |
| formatMethodHints | `formatMethodHints(method: ModuleUnit["methods"][number]): string` | string |  | llm | 将方法的入口点和框架提示信息格式化为逗号分隔的唯一字符串。 | join, method.entrypointHints.map, method.frameworkHints.map | src/docs/docsGenerator.ts:656 |
| numberedList | `numberedList(items: string[]): string` | string |  | llm | 将字符串数组转换为带编号的列表字符串，若数组为空则返回默认提示。 | items.map, join | src/docs/docsGenerator.ts:669 |
| renderCallGraph | `renderCallGraph(result: AnalysisResult): string` | string |  | llm | 根据分析结果生成包含Mermaid流程图和调用边表格的调用图文档字符串。 | String, callEdges.forEach, callEdges.map, escapeMermaid, heading, join, labels.get, mermaidLines.push | src/docs/docsGenerator.ts:677 |
...
src/docs/qualityReport.ts 包含 0 个类、10 个方法单元和 4 个导入。 关键方法：根据分析结果、语义概览、项目叙事和文档生成质量报告的字符串表示。 根据分析结果、语义概览、项目叙事和文档生成质量摘要，包含检查项、评分和推荐建议。 buildChecks 定义一个可调用单元；调用 countMatches, docs.values, hasRequiredDocs, isChineseNarrative, join。
...
| calculateScore | `calculateScore(checks: QualityCheck[]): number` | number |  | llm | 根据质量检查结果计算平均得分，通过将每个检查的状态映射为分数（通过100分，警告60分，失败0分）并取平均值。 | Math.round, checks.reduce | src/docs/qualityReport.ts:197 |
| recommendations | `recommendations(checks: QualityCheck[], overview: SemanticOverview): string[]` | string[] |  | llm | 根据质量检查结果和语义概览生成工程文档改进建议列表。 | checks.some, items.push | src/docs/qualityReport.ts:211 |
| countMatches | `countMatches(text: string, patterns: string[]): number` | number |  | llm | 计算给定文本中所有模式字符串出现次数的总和。 | patterns.reduce, text.split | src/docs/qualityReport.ts:230 |
| isChineseNarrative | `isChineseNarrative(value: string): boolean` | boolean |  | llm | 判断字符串是否包含中文字符且长度大于20。 | test | src/docs/qualityReport.ts:234 |
| maxDocLength | `maxDocLength(docs: Map<string, string>): number` | number |  | llm | 计算文档集合中所有文档内容的最大长度。 | Math.max, docs.values, map | src/docs/qualityReport.ts:238 |
| hasRequiredDocs | `hasRequiredDocs(docs: Map<string, string>): boolean` | boolean |  | heuristic | hasRequiredDocs 定义一个可调用单元；调用 docs.has, every；访问 FILE:README.md, FILE:ai/AI_CONTEXT.md, FILE:deep-dive/CALL_GRAPH.md, FILE:deep-dive/DATA_AND_RESOURCES.md, FILE:deep-dive/ENTRYPOINTS.md。 | docs.has, every | src/docs/qualityReport.ts:242 |
...
src/index.ts 包含 0 个类、2 个方法单元和 6 个导入。 关键方法：main 定义一个可调用单元；调用 normalizeProvider, parseCliArgs, printHelp, process.argv.slice, runAnalyzeCommand。 printHelp 定义一个可调用单元；调用 console.log。
...
| Method | Signature | Return | Hints | Analyzer | Summary | Calls | Location |
| --- | --- | --- | --- | --- | --- | --- | --- |
| main | `main(): Promise<void>` | Promise<void> | cli | heuristic | main 定义一个可调用单元；调用 normalizeProvider, parseCliArgs, printHelp, process.argv.slice, runAnalyzeCommand。 | normalizeProvider, parseCliArgs, printHelp, process.argv.slice, runAnalyzeCommand, runDoctorCommand, runInitCommand, runInteractiveCommand | src/index.ts:9 |
| printHelp | `printHelp(): void` | void |  | heuristic | printHelp 定义一个可调用单元；调用 console.log。 | console.log | src/index.ts:54 |
...
src/llm/modelConfig.ts 包含 0 个类、9 个方法单元和 3 个导入。 关键方法：loadModelConfig 定义一个可调用单元；调用 Boolean, defaultBaseUrl, defaultModel, getApiKey, modelProviderSchema.parse。 applyModelNetworkEnv 定义一个可调用单元；访问 ENV:NO_PROXY, ENV:no_proxy。 将ModelConfig对象转换为ModelRuntimeInfo对象，并根据provider字段决定是否保留model字段。
...
| Method | Signature | Return | Hints | Analyzer | Summary | Calls | Location |
| --- | --- | --- | --- | --- | --- | --- | --- |
| loadModelConfig | `loadModelConfig(env: NodeJS.ProcessEnv = process.env, config: ProjectConfig["llm"] = {}): ModelConfig` | ModelConfig |  | heuristic | loadModelConfig 定义一个可调用单元；调用 Boolean, defaultBaseUrl, defaultModel, getApiKey, modelProviderSchema.parse。 | Boolean, defaultBaseUrl, defaultModel, getApiKey, modelProviderSchema.parse, parseInteger, parseNumber, parseOptionalInteger | src/llm/modelConfig.ts:30 |
| applyModelNetworkEnv | `applyModelNetworkEnv(config: ModelConfig): void` | void | environment:NO_PROXY, environment:no_proxy | heuristic | applyModelNetworkEnv 定义一个可调用单元；访问 ENV:NO_PROXY, ENV:no_proxy。 |  | src/llm/modelConfig.ts:63 |
| toModelRuntimeInfo | `toModelRuntimeInfo(config: ModelConfig): ModelRuntimeInfo` | ModelRuntimeInfo |  | llm | 将ModelConfig对象转换为ModelRuntimeInfo对象，并根据provider字段决定是否保留model字段。 |  | src/llm/modelConfig.ts:72 |
| defaultModel | `defaultModel(provider: ModelProvider): string` | string |  | llm | 根据提供商返回默认模型名称。 |  | src/llm/modelConfig.ts:83 |
| defaultBaseUrl | `defaultBaseUrl(provider: ModelProvider): string \| undefined` | string \| undefined |  | llm | 根据模型提供商返回默认的基础URL，目前仅支持DeepSeek。 |  | src/llm/modelConfig.ts:99 |
| parseNumber | `parseNumber(value: string \| undefined, fallback: number): number` | number |  | llm | 解析字符串为数字，若无效则返回默认值。 | Number, Number.isFinite | src/llm/modelConfig.ts:128 |
| parseInteger | `parseInteger(value: string \| undefined, fallback: number): number` | number |  | llm | 解析字符串为整数，若无效则返回默认值。 | Number.isFinite, Number.parseInt | src/llm/modelConfig.ts:137 |
| parseOptionalInteger | `parseOptionalInteger(value: string \| undefined): number \| undefined` | number \| undefined |  | llm | 解析可选字符串参数为非负整数，若无效则返回 undefined。 | Number.isFinite, Number.parseInt | src/llm/modelConfig.ts:146 |
## src/llm/modelFactory.ts

src/llm/modelFactory.ts 包含 0 个类、1 个方法单元和 4 个导入。 关键方法：createChatModel 定义一个可调用单元；调用 applyModelNetworkEnv。
...
| Method | Signature | Return | Hints | Analyzer | Summary | Calls | Location |
| --- | --- | --- | --- | --- | --- | --- | --- |
| createChatModel | `createChatModel(config: ModelConfig): ChatModel \| undefined` | ChatModel \| undefined |  | heuristic | createChatModel 定义一个可调用单元；调用 applyModelNetworkEnv。 | applyModelNetworkEnv | src/llm/modelFactory.ts:10 |
## src/output/resultJsonWriter.ts

src/output/resultJsonWriter.ts 包含 0 个类、32 个方法单元和 6 个导入。 关键方法：writeResultJson 定义一个可调用单元；调用 JSON.stringify, buildResultDiff, fs.mkdir, fs.writeFile, path.dirname；访问 FILE:CHANGE_SUMMARY.md, FILE:result-diff.json, FILE:result.json。 将扫描结果、概览、质量数据和文档路径组装为结构化的 JSON 对象并返回。 将模块单元序列化为包含标识符、路径、语言、导入、摘要以及类和方法的ID列表的普通对象。
...
| Method | Signature | Return | Hints | Analyzer | Summary | Calls | Location |
| --- | --- | --- | --- | --- | --- | --- | --- |
| writeResultJson | `writeResultJson(input: ResultJsonInput): Promise<ResultJsonWriteResult>` | Promise<ResultJsonWriteResult> |  | heuristic | writeResultJson 定义一个可调用单元；调用 JSON.stringify, buildResultDiff, fs.mkdir, fs.writeFile, path.dirname；访问 FILE:CHANGE_SUMMARY.md, FILE:result-diff.json, FILE:result.json。 | JSON.stringify, buildResultDiff, fs.mkdir, fs.writeFile, path.dirname, path.join, readJson, renderChangeSummary | src/output/resultJsonWriter.ts:98 |

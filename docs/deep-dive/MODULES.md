...
| src/cli/analyzeCommand.ts | typescript | 0 | 1 | ../analyzer/analyzeRepo.js, ../config/projectConfig.js, ../docs/docsGenerator.js, ../llm/modelConfig.js, ../output/resultJsonWriter.js, node:path | src/cli/analyzeCommand.ts 包含 0 个类、1 个方法单元和 6 个导入。 关键方法：runAnalyzeCommand 定义一个可调用单元；调用 analyzeRepo, console.log, generateDocs, loadModelConfig, loadProjectConfig；访问 FILE:QUALITY_REPORT.md, FILE:README.md。 |
...
| Method | Signature | Return | Hints | Analyzer | Summary | Calls | Location |
| --- | --- | --- | --- | --- | --- | --- | --- |
| runAnalyzeCommand | `runAnalyzeCommand(options: AnalyzeCommandOptions): Promise<void>` | Promise<void> |  | heuristic | runAnalyzeCommand 定义一个可调用单元；调用 analyzeRepo, console.log, generateDocs, loadModelConfig, loadProjectConfig；访问 FILE:QUALITY_REPORT.md, FILE:README.md。 | analyzeRepo, console.log, generateDocs, loadModelConfig, loadProjectConfig, path.join, path.resolve, writeResultJson | src/cli/analyzeCommand.ts:13 |
## src/cli/args.ts
